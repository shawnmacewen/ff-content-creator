...
function getSectionTabs(section: SettingsSection) {
  return section === 'workspace' ? workspaceTabs : productLabTabs;
}
...
function getInitialTab(tab: string | null, section: SettingsSection): SettingsTab {
  ...
  return sectionTabs.includes(tab as SettingsTab) ? (tab as SettingsTab) : getDefaultTab(section);
}

function getInitialTabFromLocation(section: SettingsSection): SettingsTab {
  if (typeof window === 'undefined') return getDefaultTab(section);
  return getInitialTab(new URLSearchParams(window.location.search).get('tab'), section);
}
...
export default function SettingsClient({ section }: { section: SettingsSection }) {
  const router = useRouter();
  const sectionTabs = getSectionTabs(section);
  const basePath = section === 'workspace' ? '/settings' : '/product-lab';
  const defaultTab = getDefaultTab(section);
  const [tab, setTab] = useState<SettingsTab>(() => getInitialTabFromLocation(section));

  const selectTab = (nextTab: SettingsTab) => {
    setTab(nextTab);
    router.replace(nextTab === defaultTab ? basePath : `${basePath}?tab=${nextTab}`, { scroll: false });
  };
