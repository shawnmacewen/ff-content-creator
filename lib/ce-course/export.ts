...
export function buildCeCourseExportPayload(row: Record<string, any>) {
...
export function buildCeCourseApiSummary(row: Record<string, any>) {
  const payload = buildCeCourseExportPayload(row);
  return {
    ...
    sourceCount: payload.readingList.length,
    questionCount: payload.quiz.length,
    passingScore: payload.quizRules.passingScore,
    readiness: payload.readiness,
    updatedAt: row.updated_at || null,
    createdAt: row.created_at || null,
    detailUrl: `/api/ce-course/public/packages/${payload.packageId}`,
  };
}
